...
namespace CityIdle.Core
{
    public sealed class EconomyService
    {
        ...
        {
            if ((int)id < 0 || (int)id >= BalanceCatalog.Businesses.Length) return false;
            ...
            if (!CanAfford(state.cash, cost)) return false;
            state.cash -= cost;
            unit.level++;
            unit.unlocked = true;
            return true;
        ...
        {
            if ((int)id < 0 || (int)id >= BalanceCatalog.Businesses.Length || unitIndex < 0 || unitIndex >= 12) return false;
            var def = BalanceCatalog.Businesses[(int)id];
            var unit = state.businesses[(int)id].units[unitIndex];
            var row = BalanceCatalog.Unit(def, unitIndex);
            if (!unit.unlocked || unit.level <= 0 || unit.manager || !CanAfford(state.cash, row.ManagerCost)) return false;
        ...
        {
            if (nowMs < state.lastSeenUtcMs) return 0;
        ...
        private static bool CanAfford(double cash, double cost) => !double.IsNaN(cash) && !double.IsInfinity(cash)
            && !double.IsNaN(cost) && !double.IsInfinity(cost) && cash >= cost && cost >= 0;
