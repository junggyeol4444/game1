...
namespace CityIdle.Core
{
    ...
    {
        public string day;
        public int targetTaps = 25;
        public int taps;
        public int targetBuys = 10;
        public int buys;
        public int targetManagers = 1;
        public int managers;
        public bool tapsClaimed;
        public bool buysClaimed;
        public bool managersClaimed;
    }
    ...
    public sealed class CollectionState
    {
        ...
        public List<string> seenBuildings = new List<string>();
    }

    [Serializable]
    public sealed class DailyPlayState
    {
        public string day;
        public int freeMinigames;
        public int rewardedMinigames;
    }
    ...
    public sealed class GameState
    {
        ...
        public DailyPlayState dailyPlay = new DailyPlayState();
        public PurchaseState purchases = new PurchaseState();
    }
    ...
    public static class BalanceCatalog
    {
        ...
        public static GameState Normalize(GameState state, long nowMs)
        {
            ...
            state.mission = state.mission ?? new MissionState();
            state.attendance = state.attendance ?? new AttendanceState();
            state.collection = state.collection ?? new CollectionState();
            state.collection.discoveries = state.collection.discoveries ?? new List<string>();
            state.collection.seenBuildings = state.collection.seenBuildings ?? new List<string>();
            state.dailyPlay = state.dailyPlay ?? new DailyPlayState();
            state.purchases = state.purchases ?? new PurchaseState();
            state.purchases.ownedProducts = state.purchases.ownedProducts ?? new List<string>();
            if (double.IsNaN(state.cash) || double.IsInfinity(state.cash) || state.cash < 0) state.cash = 20;
            if (double.IsNaN(state.material) || double.IsInfinity(state.material) || state.material < 0) state.material = 0;
            if (double.IsNaN(state.ore) || double.IsInfinity(state.ore) || state.ore < 0) state.ore = 0;
            if (double.IsNaN(state.goods) || double.IsInfinity(state.goods) || state.goods < 0) state.goods = 0;
            if (double.IsNaN(state.food) || double.IsInfinity(state.food) || state.food < 0) state.food = 0;
            if (double.IsNaN(state.visitors) || double.IsInfinity(state.visitors) || state.visitors < 0) state.visitors = 0;
