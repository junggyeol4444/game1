...
namespace CityIdle.Core
{
    public sealed class EraService
    {
        ...
        public bool Advance(GameState state, long nowMs)
        {
            ...
            fresh.attendance = state.attendance;
            fresh.mission = state.mission;
            fresh.dailyPlay = state.dailyPlay;
            fresh.purchases = state.purchases;
            fresh.taxTotal = state.taxTotal;
            Copy(fresh, state);
            return true;
        }
        ...
        private static void Copy(GameState source, GameState target)
        {
            ...
            target.collection = source.collection; target.attendance = source.attendance; target.mission = source.mission;
            target.dailyPlay = source.dailyPlay; target.purchases = source.purchases;
        }
    }
}
