...
namespace CityIdle.Runtime
{
    public static class RuntimeBootstrap
    {
        ...
        private static void BuildUi(CityIdleGame game)
        {
            ...
            var retentionPanel = Panel(canvas.transform, "미션·출석·도감", new Vector2(.02f, .11f), new Vector2(.98f, .88f));
            retentionPanel.SetActive(false);
            var minigamePanel = Panel(canvas.transform, "미니게임", new Vector2(.02f, .11f), new Vector2(.98f, .88f));
            minigamePanel.SetActive(false);
            var selectedRewardBusiness = BusinessId.Mine;
            ...
            facilitiesTab.onClick.AddListener(() =>
            {
                ...
                retentionPanel.SetActive(false); minigamePanel.SetActive(false);
                facilityPanel.SetActive(true);
            });
            var missionLabels = new Text[3];
            var missionNames = new[] { "사업 가동", "건설·레벨업", "매니저 고용" };
            for (var i = 0; i < 3; i++)
            {
                var top = .94f - i * .13f; var bottom = top - .1f;
                missionLabels[i] = Label(retentionPanel.transform, missionNames[i], 28, TextAnchor.MiddleLeft);
                SetRect(missionLabels[i].rectTransform, new Vector2(.04f, bottom), new Vector2(.68f, top));
                var claim = Button(retentionPanel.transform, "받기", new Vector2(.72f, bottom), new Vector2(.96f, top));
                var index = i; claim.onClick.AddListener(() => game.ClaimMission(index));
            }
            var attendance = Button(retentionPanel.transform, "오늘 출석 보상", new Vector2(.08f, .36f), new Vector2(.92f, .46f));
            attendance.onClick.AddListener(game.ClaimAttendance);
            var collection = Label(retentionPanel.transform, "Collection", 29, TextAnchor.MiddleCenter);
            SetRect(collection.rectTransform, new Vector2(.06f, .16f), new Vector2(.94f, .31f));

            for (var i = 0; i < BalanceCatalog.Businesses.Length; i++)
            {
                var def = BalanceCatalog.Businesses[i];
                var top = .95f - i * .115f; var bottom = top - .09f;
                var start = Button(minigamePanel.transform, $"{def.Name} 미니게임", new Vector2(.08f, bottom), new Vector2(.92f, top));
                var id = def.Id;
                start.onClick.AddListener(() => { selectedRewardBusiness = id; game.StartMinigame(id); });
            }
            var minigameTap = Button(minigamePanel.transform, "타이밍 탭!", new Vector2(.12f, .22f), new Vector2(.88f, .34f));
            minigameTap.onClick.AddListener(game.TapMinigame);
            var minigameStatus = Label(minigamePanel.transform, "Minigame Status", 30, TextAnchor.MiddleCenter);
            SetRect(minigameStatus.rectTransform, new Vector2(.06f, .07f), new Vector2(.94f, .19f));
            var rewardedPlay = Button(minigamePanel.transform, "선택 사업 광고 1판", new Vector2(.12f, .36f), new Vector2(.88f, .45f));
            rewardedPlay.onClick.AddListener(() => game.StartRewardedMinigame(selectedRewardBusiness));

            var retentionTab = Button(canvas.transform, "미션", new Vector2(.31f, .885f), new Vector2(.49f, .925f));
            retentionTab.onClick.AddListener(() =>
            {
                for (var p = 0; p < panels.Length; p++) if (panels[p]) panels[p].SetActive(false);
                facilityPanel.SetActive(false); minigamePanel.SetActive(false); retentionPanel.SetActive(true);
            });
            var minigameTab = Button(canvas.transform, "게임", new Vector2(.50f, .885f), new Vector2(.69f, .925f));
            minigameTab.onClick.AddListener(() =>
            {
                for (var p = 0; p < panels.Length; p++) if (panels[p]) panels[p].SetActive(false);
                facilityPanel.SetActive(false); retentionPanel.SetActive(false); minigamePanel.SetActive(true);
            });
            ...
            game.Changed += () =>
            {
                ...
                var mission = game.State.mission;
                missionLabels[0].text = $"사업 가동 {mission.taps}/{mission.targetTaps}";
                missionLabels[1].text = $"건설·레벨업 {mission.buys}/{mission.targetBuys}";
                missionLabels[2].text = $"매니저 고용 {mission.managers}/{mission.targetManagers}";
                attendance.interactable = !game.State.attendance.claimedToday;
                collection.text = $"발견 {game.State.collection.discoveries.Count} · 건물 도감 {game.State.collection.seenBuildings.Count}";
                if (game.ActiveMinigame != null && !game.ActiveMinigame.completed)
                    minigameStatus.text = $"{game.ActiveMinigame.kind} · {game.ActiveMinigame.remaining:0.0}초 · {game.ActiveMinigame.score}점";
                else if (game.LastMinigameResult != null)
                    minigameStatus.text = $"완료 {game.LastMinigameResult.score}점 · {game.LastMinigameResult.discovery}";
                else minigameStatus.text = $"무료 {game.State.dailyPlay.freeMinigames}/{MinigameService.DailyFreeLimit} · 광고 {game.State.dailyPlay.rewardedMinigames}/{MinigameService.DailyRewardedLimit}";
                rewardedPlay.interactable = game.Minigames.HasRewardedPlay(game.State, LocalSaveService.UtcNowMs) && !game.Monetization.Busy;
            };
        }
